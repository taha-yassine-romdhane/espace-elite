...
export default async function handler(
  ...
  try {
    ...
    // Fetch all patients (remove restrictive where clause)
    ...
    const transformedPatients = patients.map((patient) => {
      ...
      patient.medicalDevices.forEach(device => {
    ...
    // Debug: Log raw patient data to see what's actually stored
    patients.forEach(patient => {
      console.log(`Patient ${patient.firstName} ${patient.lastName}:`);
      console.log(`  - addressCoordinates type: ${typeof patient.addressCoordinates}`);
      console.log(`  - addressCoordinates value:`, patient.addressCoordinates);
      console.log(`  - governorate: ${patient.governorate}`);
    });
    
    console.log(`Returning ${transformedPatients.length} transformed patients`);
    
    // Debug: Log location mapping details
    const locationMappingDebug = transformedPatients.map(p => {
      const originalPatient = patients.find(pat => pat.id === p.id);
      const hasRealCoordinates = p.latitude !== 36.8065 || p.longitude !== 10.1815;
      const addressCoords = originalPatient?.addressCoordinates;
      const hasAddressCoordinates = addressCoords && typeof addressCoords === 'object' && 
                                  ((addressCoords as any).lat || (addressCoords as any).latitude);
      
      return {
        name: p.name,
        originalGovernorate: originalPatient?.governorate || 'N/A',
        normalizedRegion: p.region,
        hasRealCoordinates,
        hasAddressCoordinates,
        addressCoordinatesRaw: addressCoords,
        coordinateSource: hasRealCoordinates ? (hasAddressCoordinates ? 'addressCoordinates' : 'governorate_offset') : 'default_tunis',
        lat: p.latitude.toFixed(6),
        lng: p.longitude.toFixed(6)
      };
    });
    
    console.log(`Using real coordinates for ${locationMappingDebug.filter(p => p.hasRealCoordinates).length}/${transformedPatients.length} patients`);
    console.log('Location mapping details:', JSON.stringify(locationMappingDebug, null, 2));
