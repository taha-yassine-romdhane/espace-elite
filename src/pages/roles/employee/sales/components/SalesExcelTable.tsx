...
export default function SalesExcelTable() {
  ...
  const handleStartAddNew = () => {
    ...
    // Auto-assign to logged-in employee
    setNewSaleData({
      ...newSaleData,
  ...
  const handleViewDetails = (sale: Sale) => {
    router.push(`/roles/employee/sales/${sale.id}`);
  };
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-center border-r border-slate-100">
                      <Button
                        ...
                        className={`h-8 px-2 ${(sale.items?.length || 0) > 0 ? 'bg-green-50 text-green-700 hover:bg-green-100' : 'text-slate-500 hover:bg-slate-100'}`}
                      >
          ...
          <div className="space-y-4">
            {selectedSale?.payment ? (
              <div className="space-y-3">
                {/* Payment Summary */}
                <div className="border rounded-lg p-4 bg-gradient-to-br from-green-50 to-indigo-50">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedSale.payment.paymentDetails.map((detail, index) => {
                        ...
                        return (
                          <div key={detail.id || index} className={`border rounded-lg p-3 transition-colors ${isCNAM ? 'bg-green-50 hover:bg-green-100 border-green-200' : 'bg-slate-50 hover:bg-slate-100'}`}>
                            <div className="flex justify-between items-start mb-2">
                              <div className="flex items-center gap-2">
                                <Badge variant="outline" className={`font-medium ${isCNAM ? 'bg-green-100 text-green-700 border-blue-300' : ''}`}>
                                  {detail.method}
                                </Badge>
                                {detail.paymentCode && (
                            ...
                            <div className="flex flex-wrap gap-3 text-xs text-slate-600">
                              ...
                              {isCNAM && dossierNumber && (
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="font-medium text-green-700">N° CNAM: {dossierNumber}</span>
                                </div>
                              )}
                              {isCNAM && bonType && (
                                <div className="flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {item.medicalDeviceId ? (
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <Badge className="font-mono bg-green-100 text-green-700 border-blue-300">
            ...
            {selectedSale?.cnamBons && selectedSale.cnamBons.length > 0 ? (
              <div className="space-y-3">
                {selectedSale.cnamBons.map((bon, index) => (
                  <div key={bon.id} className="border rounded-lg p-4 bg-gradient-to-r from-purple-50 to-green-50">
                    {/* Header Section */}
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
