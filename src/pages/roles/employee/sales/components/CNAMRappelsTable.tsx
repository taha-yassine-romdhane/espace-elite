...
interface CNAMRappelsTableProps {
  showActions?: boolean;
}

export default function CNAMRappelsTable({ showActions = true }: CNAMRappelsTableProps) {
  ...
  const rappelRows: CNAMRappelRow[] = useMemo(() => {
    ...
    return salesData.map((sale: any) => {
      ...
      return {
        ...
        patientActive: sale.patient?.isActive,
          ...
          <table className="w-full border-collapse">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr className="border-b border-slate-200">
                ...
                <th className="px-3 py-3 text-left text-xs font-semibold text-slate-700 border-r border-slate-200 min-w-[150px]">Paiements</th>
                <th className="px-3 py-3 text-right text-xs font-semibold text-slate-700 border-r border-slate-200 min-w-[140px]">Montants</th>
            ...
            <tbody>
              {paginatedRows.length === 0 ? (
                <tr>
                  <td colSpan={showActions ? 10 : 9} className="px-3 py-12 text-center">
                ...
                >
                        ...
                        <div
                          ...
                          onClick={() => {
                            if (row.clientType === 'patient') {
                              router.push(`/roles/employee/renseignement/patient/${row.clientId}`);
                            }
                          }}
                        >
                          {row.clientName}
                  ...
                  {/* Payment Codes and Methods - Compacted */}
                  <td className="px-3 py-2.5 text-xs border-r border-slate-100">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-xs font-semibold text-green-700">
                          {row.totalPaid ? formatCurrency(Number(row.totalPaid)) : '-'}
                        </span>
                      </div>
                      {/* Sale Amount */}
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-xs font-semibold text-slate-700">
                          {row.saleAmount ? formatCurrency(Number(row.saleAmount)) : '-'}
                        </span>
                      </div>
                      {/* Remaining */}
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className={`text-xs font-semibold ${
                          ...
                          {row.remainingAmount !== undefined ? formatCurrency(Number(row.remainingAmount)) : '-'}
                        </span>
                      </div>
                    </div>
                  </td>
