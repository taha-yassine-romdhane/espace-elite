  ...
  const renderCell = (diagnostic: Diagnostic, field: string, isEditing: boolean) => {
    ...
    if (isEditing && editedDiagnostic) {
      switch (field) {
        ...
        case 'patientWithCode':
          // Combined patient name and code in edit mode
          return (
            <div className="flex flex-col gap-1.5">
              ...
              {(() => {
                ...
                const editIsActive = (editedDiagnostic.patient as any)?.isActive;
                return editPatientCode ? (
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500 font-mono">{editPatientCode}</span>
                    {editIsActive === true ? (
        ...
        case 'dateWithFollowUp':
          // Combined date and follow-up in edit mode
          const editDiagnosticDateValue = editedDiagnostic.diagnosticDate
            ? new Date(editedDiagnostic.diagnosticDate).toISOString().split('T')[0]
            : '';
          const editFollowUpDateValue = editedDiagnostic.followUpDate
            ? new Date(editedDiagnostic.followUpDate).toISOString().split('T')[0]
            : '';
          return (
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-slate-600 font-medium whitespace-nowrap">Date:</span>
                <Input
                  type="date"
                  value={editDiagnosticDateValue}
                  onChange={(e) => updateEditedField('diagnosticDate', e.target.value ? new Date(e.target.value) : null)}
                  className="h-7 text-xs"
                />
              </div>
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-slate-600 font-medium whitespace-nowrap">Suivi:</span>
                <Input
                  type="date"
                  value={editFollowUpDateValue}
        ...
        case 'iahAndId':
          // Combined IAH and ID in edit mode
          return (
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-slate-600 font-medium whitespace-nowrap">IAH:</span>
                <Input
                  type="number"
                  step="0.1"
              ...
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-slate-600 font-medium whitespace-nowrap">ID:</span>
                <Input
                  type="number"
                  step="0.1"
        ...
        case 'severityAndEquipped':
          // Combined severity and equipment status - read-only in edit mode
          const editSeverityData = getSeverity(editedDiagnostic.result?.iah, editedDiagnostic.hasSale, editedDiagnostic.hasRental);
          const editEquipmentStatusData = getEquipmentStatus(editedDiagnostic.hasSale, editedDiagnostic.hasRental);
          return (
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-slate-500 font-medium">Sévérité:</span>
                <Badge variant="outline" className={`text-xs ${editSeverityData.color}`}>
                  {editSeverityData.label}
                </Badge>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-slate-500 font-medium">Appareillé:</span>
                <Badge variant="outline" className={`text-xs ${editEquipmentStatusData.color}`}>
                  {editEquipmentStatusData.label}
                </Badge>
              </div>
            </div>
          );
    ...
    switch (field) {
      ...
      case 'patientWithCode':
        ...
        const isActive = (diagnostic.patient as any)?.isActive;

        if (!diagnostic.patient?.id || patientName === '-') {
          return (
            <div className="flex flex-col gap-1">
              <span className="text-xs whitespace-nowrap">{patientName}</span>
              {patientCode && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-500 font-mono">{patientCode}</span>
                  {isActive === true ? (
                    <Badge className="bg-green-100 text-green-700 border-green-300 text-xs px-1.5 py-0">
                      Actif
                    </Badge>
          ...
          <div className="flex flex-col gap-1">
            ...
            {patientCode && (
              <div className="flex items-center gap-2">
                <Link
                  href={`/roles/employee/renseignement/patient/${diagnostic.patient.id}`}
                  className="text-xs text-gray-500 font-mono hover:text-gray-700"
                >
                  {patientCode}
                </Link>
                {isActive === true ? (
      ...
      case 'patientId':
        const patientNameOnly = diagnostic.patient
          ? `${diagnostic.patient.firstName || ''} ${diagnostic.patient.lastName || ''}`.trim()
          : '-';
        if (!diagnostic.patient?.id || patientNameOnly === '-') {
          return <span className="text-xs whitespace-nowrap">{patientNameOnly}</span>;
        }
        return (
          <Link
            href={`/roles/employee/renseignement/patient/${diagnostic.patient.id}`}
      ...
      case 'patientCode':
        const patientCodeOnly = (diagnostic.patient as any)?.patientCode;
        if (!patientCodeOnly || !diagnostic.patient?.id) {
      ...
      case 'dateWithFollowUp':
        ...
        const followUpDateValue = diagnostic.followUpDate;
        ...
        return (
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1.5">
              ...
              {diagnosticDateValue ? (
                <span className="text-xs font-semibold">{format(new Date(diagnosticDateValue), 'dd/MM/yyyy', { locale: fr })}</span>
              ) : (
                <span className="text-xs">-</span>
              )}
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-slate-500 font-medium">Suivi:</span>
              {followUpDateValue ? (
                <span className="text-xs text-orange-700 font-semibold">{format(new Date(followUpDateValue), 'dd/MM/yyyy', { locale: fr })}</span>
              ) : (
                <span className="text-xs">-</span>
              )}
            </div>
      ...
      case 'iahAndId':
        const iahValue = diagnostic.result?.iah;
        const idValueData = diagnostic.result?.idValue;

        return (
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-slate-500 font-medium">IAH:</span>
              <span className="text-xs font-semibold">
                {iahValue !== null && iahValue !== undefined ? iahValue.toFixed(1) : '-'}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-slate-500 font-medium">ID:</span>
              <span className="text-xs font-semibold">
                {idValueData !== null && idValueData !== undefined ? idValueData.toFixed(1) : '-'}
              </span>
            </div>
          </div>
        );
      ...
      case 'diagnosticDate':
        const diagDateValue = diagnostic.diagnosticDate || diagnostic.date;
        if (!diagDateValue) return <span className="text-xs">-</span>;
        const date = new Date(diagDateValue);
        return (
      ...
      case 'severityAndEquipped':
        const severityData = getSeverity(diagnostic.result?.iah, diagnostic.hasSale, diagnostic.hasRental);
        const equipmentStatusData = getEquipmentStatus(diagnostic.hasSale, diagnostic.hasRental);

        return (
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-slate-500 font-medium">Sévérité:</span>
              <Badge variant="outline" className={`text-xs ${severityData.color}`}>
                {severityData.label}
              </Badge>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-slate-500 font-medium">Appareillé:</span>
              <Badge variant="outline" className={`text-xs ${equipmentStatusData.color}`}>
                {equipmentStatusData.label}
              </Badge>
            </div>
          </div>
        );
    ...
    <tr className="bg-blue-50 border-b">
      ...
      <td className="px-2 py-2">
        <div className="flex flex-col gap-1.5">
          <PatientSelector
            ...
            className="h-8 text-xs"
          />
          <span className="text-xs text-gray-500 font-mono">Auto</span>
        </div>
      </td>
      <td className="px-2 py-2">
      ...
      <td className="px-2 py-2">
        {(() => {
          ...
          const newEquipmentStatus = getEquipmentStatus(false, false);
          return (
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-slate-500 font-medium">Sévérité:</span>
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-slate-500 font-medium">Appareillé:</span>
                <Badge variant="outline" className={`text-xs ${newEquipmentStatus.color}`}>
                  {newEquipmentStatus.label}
                </Badge>
              </div>
            </div>
          );
          ...
          <table className="text-sm w-full">
            <thead className="bg-gray-100 border-b sticky top-0">
              <tr>
                ...
                <th className="px-2 py-3 text-left font-medium text-xs whitespace-nowrap">IAH / ID</th>
                ...
                paginatedDiagnostics.map((diagnostic: Diagnostic) => {
                  ...
                  return (
                    <tr key={diagnostic.id} className={`border-b ${isEditing ? 'bg-yellow-50' : 'hover:bg-gray-50'}`}>
                      ...
                      <td className="px-2 py-2">{renderCell(diagnostic, 'patientWithCode', isEditing)}</td>
                      ...
                      <td className="px-2 py-2">{renderCell(diagnostic, 'dateWithFollowUp', isEditing)}</td>
                      <td className="px-2 py-2">{renderCell(diagnostic, 'status', isEditing)}</td>
                      <td className="px-2 py-2">{renderCell(diagnostic, 'iahAndId', isEditing)}</td>
                      <td className="px-2 py-2">{renderCell(diagnostic, 'severityAndEquipped', isEditing)}</td>
