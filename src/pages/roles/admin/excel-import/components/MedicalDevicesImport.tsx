...
const medicalDeviceFields: MedicalDeviceField[] = [
  { 
    key: 'name', 
    label: 'Nom de l\'appareil', 
    required: true, 
    type: 'string',
    description: 'Nom de l\'appareil médical',
    example: 'CPAP ResMed AirSense 10'
  },
  { 
    key: 'type', 
    ...
    required: true, 
    type: 'select',
    description: 'Type d\'appareil médical',
    example: 'CPAP',
    options: ['CPAP', 'VNI', 'CONCENTRATEUR_OXYGENE', 'MASQUE', 'AUTRE']
  },
  { 
    key: 'brand', 
    label: 'Marque', 
  ...
  { 
    ...
    required: false, 
    type: 'number',
    description: 'Prix de location mensuel en dinars',
    example: '150.00'
  },
  { 
  ...
  { 
    ...
    required: false, 
    type: 'string',
    description: 'Informations sur la garantie',
    example: '24 mois'
  },
  { 
    key: 'technicalSpecs', 
  ...
  { 
    ...
    required: false, 
    type: 'string',
    description: 'Intervalle de maintenance recommandé',
    example: '6 mois'
  },
  { 
    key: 'location', 
    ...
    required: false, 
    type: 'string',
    description: 'Localisation actuelle de l\'appareil'
  },
  { 
    key: 'status', 
    label: 'Statut', 
    ...
    example: 'ACTIVE',
    options: ['ACTIVE', 'MAINTENANCE', 'RETIRED', 'RESERVED', 'SOLD']
  },
  { 
    key: 'destination', 
    label: 'Destination', 
    required: false, 
    type: 'select',
    description: 'Destination de l\'appareil (Vente ou Location)',
    example: 'FOR_RENT',
...
export default function MedicalDevicesImport() {
        ...
        if (columnIndex !== -1) {
          ...
          if (['installationDate'].includes(fieldKey) && value) {
            if (typeof value === 'number') {
              ...
              device[fieldKey] = date.toISOString().split('T')[0];
  ...
  const handleImport = async () => {
    ...
    try {
      for (let i = 0; i < totalBatches; i++) {
        ...
        const response = await fetch('/api/excel-import/medical-devices', {
          method: 'POST',
      ...
      toast({
        ...
        description: `${result.success} appareils médicaux importés avec succès`,
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await fetch('/api/excel-import/medical-devices/template');
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'template_medical_devices_import.xlsx';
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <CardTitle>Import d'Appareils Médicaux</CardTitle>
          </div>
          ...
          <div className="space-y-6">
            <div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
              ...
              <h3 className="text-lg font-medium mb-2">
                Sélectionnez votre fichier Excel d'appareils médicaux
              </h3>
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>Types d'appareils supportés:</strong> CPAP, VNI, Concentrateur d'oxygène, Masques et autres.
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {mappedData.length} appareils médicaux prêts à être importés
              </p>
            </div>
                  ...
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      {Object.entries(fieldMapping).map(([fieldKey]) => {
                        const field = medicalDeviceFields.find(f => f.key === fieldKey);
                        return (
        ...
        {step === 'importing' && (
          <div className="space-y-6 py-12">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">
                Veuillez patienter pendant l'import des appareils médicaux
              </p>
            </div>
        ...
        {step === 'complete' && importResult && (
          <div className="space-y-6">
            <div className="text-center py-8">
              ...
              <div className="space-y-2">
                <p className="text-green-600">
                  <strong>{importResult.success}</strong> appareils médicaux importés avec succès
                </p>
                {importResult.failed > 0 && (
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <Button onClick={() => window.location.href = '/roles/admin/medical-devices'}>
