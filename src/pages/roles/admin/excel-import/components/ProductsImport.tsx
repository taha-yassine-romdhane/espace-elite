...
const productFields: ProductField[] = [
  ...
  { 
    ...
    required: true, 
    type: 'select',
    description: 'Type de produit (Accessoire ou Pièce détachée)',
    example: 'ACCESSORY',
    options: ['ACCESSORY', 'SPARE_PART']
  },
  { 
    key: 'brand', 
    label: 'Marque', 
  ...
  { 
    ...
    required: false, 
    type: 'string',
    description: 'Numéro de série unique du produit',
    example: 'SN123456789'
  },
  { 
    key: 'purchasePrice', 
  ...
  { 
    ...
    required: false, 
    type: 'date',
    description: 'Date d\'expiration de la garantie',
    example: '01/01/2026'
  },
  { 
    key: 'status', 
  ...
  { 
    ...
    required: false, 
    type: 'string',
    description: 'Notes ou remarques sur le produit'
  },
  { 
...
export default function ProductsImport() {
  ...
  const handleMappingComplete = (mapping: Record<string, string>) => {
    ...
    const transformed = excelData.map((row, index) => {
      const product: any = { _rowIndex: index + 2 };
        ...
        if (columnIndex !== -1) {
          ...
          if (['purchaseDate', 'warrantyExpiration'].includes(fieldKey) && value) {
            if (typeof value === 'number') {
              ...
              product[fieldKey] = date.toISOString().split('T')[0];
            } else if (typeof value === 'string') {
              product[fieldKey] = value;
            }
          } 
          // Handle number fields
          else if (['purchasePrice', 'sellingPrice'].includes(fieldKey) && value) {
            product[fieldKey] = parseFloat(value) || 0;
          }
          // Handle other fields
          else {
            product[fieldKey] = value;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${result.success} produits importés avec succès`,
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await fetch('/api/excel-import/products/template');
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'template_products_import.xlsx';
          ...
          <div className="space-y-6">
            <div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
              ...
              <h3 className="text-lg font-medium mb-2">
                Sélectionnez votre fichier Excel de produits
              </h3>
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>Types de produits supportés:</strong> Accessoires et Pièces détachées.
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {mappedData.length} produits prêts à être importés
              </p>
            </div>
        ...
        {step === 'complete' && importResult && (
          <div className="space-y-6">
            <div className="text-center py-8">
              ...
              <div className="space-y-2">
                <p className="text-green-600">
                  <strong>{importResult.success}</strong> produits importés avec succès
                </p>
                {importResult.failed > 0 && (
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <Button onClick={() => window.location.href = '/roles/admin/stock'}>
