...
const sparePartFields: SparePartField[] = [
  { 
    key: 'name', 
    label: 'Nom de la pièce', 
    required: true, 
    type: 'string',
    description: 'Nom de la pièce de rechange',
    example: 'Moteur - Turbine'
  },
  { 
    key: 'brand', 
    label: 'Marque', 
    required: false, 
    type: 'string',
    description: 'Marque de la pièce',
    example: 'Generic'
  },
  { 
    key: 'model', 
    ...
    required: false, 
    type: 'string',
    description: 'Référence ou modèle de la pièce',
    example: 'GenMot0001'
  },
  { 
...
export default function SparePartsImport() {
  ...
  const handleMappingComplete = (mapping: Record<string, string>) => {
    ...
    const transformed = excelData.map((row, index) => {
      const sparePart: any = { _rowIndex: index + 2 };
      ...
      Object.entries(mapping).forEach(([fieldKey, excelColumn]) => {
        ...
        if (columnIndex !== -1) {
          ...
          if (['purchasePrice', 'sellingPrice', 'stockQuantity'].includes(fieldKey) && value) {
            sparePart[fieldKey] = parseFloat(value) || 0;
          }
          // Handle other fields
          else {
            sparePart[fieldKey] = value;
          }
        }
      });
  ...
  const handleImport = async () => {
    ...
    try {
      for (let i = 0; i < totalBatches; i++) {
        ...
        const response = await fetch('/api/excel-import/spare-parts', {
          method: 'POST',
      ...
      toast({
        ...
        description: `${result.success} pièces de rechange importées avec succès`,
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await fetch('/api/excel-import/spare-parts/template');
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'template_spare_parts_import.xlsx';
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <CardTitle>Import de Pièces de Rechange</CardTitle>
          </div>
          ...
          <div className="space-y-6">
            <div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
              ...
              <h3 className="text-lg font-medium mb-2">
                Sélectionnez votre fichier Excel de pièces de rechange
              </h3>
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>Types de pièces supportés:</strong> Moteurs, Cartes électroniques, Capteurs, Valves, Joints, Connecteurs, Écrans, Boutons, etc.
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {mappedData.length} pièces de rechange prêtes à être importées
              </p>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      {Object.entries(fieldMapping).map(([fieldKey]) => {
                        const field = sparePartFields.find(f => f.key === fieldKey);
                        return (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {mappedData.slice(0, 10).map((sparePart, index) => (
                      <tr key={index}>
                        ...
                        {Object.keys(fieldMapping).map(fieldKey => (
                          <td key={fieldKey} className="px-4 py-3 text-sm text-gray-900">
                            {typeof sparePart[fieldKey] === 'boolean' ? (sparePart[fieldKey] ? 'Oui' : 'Non') : (sparePart[fieldKey] || '-')}
                          </td>
                        ))}
                      </tr>
                    ))}
        ...
        {step === 'importing' && (
          <div className="space-y-6 py-12">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">
                Veuillez patienter pendant l'import des pièces de rechange
              </p>
            </div>
            ...
            <div className="text-center py-8">
              ...
              <div className="space-y-2">
                <p className="text-green-600">
                  <strong>{importResult.success}</strong> pièces de rechange importées avec succès
                </p>
                {importResult.failed > 0 && (
