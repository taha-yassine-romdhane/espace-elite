...
const accessoryFields: AccessoryField[] = [
  { 
    key: 'name', 
  ...
  { 
    ...
    required: false, 
    type: 'string',
    description: 'Marque de l\'accessoire',
    example: 'ResMed'
  },
  { 
    key: 'model', 
  ...
  { 
    ...
    required: false, 
    type: 'date',
    description: 'Date de fin de garantie',
    example: '2025-12-31'
  },
  { 
    key: 'status', 
    ...
    required: false, 
    type: 'select',
    description: 'Statut de l\'accessoire',
    example: 'FOR_SALE',
    options: ['FOR_SALE', 'FOR_RENT', 'EN_REPARATION', 'HORS_SERVICE']
  }
];
...
export default function AccessoriesImport() {
  ...
  const handleMappingComplete = (mapping: Record<string, string>) => {
    ...
    const transformed = excelData.map((row, index) => {
      const accessory: any = { _rowIndex: index + 2 };
        ...
        if (columnIndex !== -1) {
          ...
          if (['warrantyExpiration'].includes(fieldKey) && value) {
            if (typeof value === 'number') {
              ...
              accessory[fieldKey] = date.toISOString().split('T')[0];
            } else if (typeof value === 'string') {
              accessory[fieldKey] = value;
            }
          } 
          // Handle number fields
          else if (['purchasePrice', 'sellingPrice', 'stockQuantity'].includes(fieldKey) && value) {
            accessory[fieldKey] = parseFloat(value) || 0;
          }
          // Handle other fields
          else {
            accessory[fieldKey] = value;
      ...
      // Ensure type is always ACCESSORY
  ...
  const handleImport = async () => {
    ...
    try {
      for (let i = 0; i < totalBatches; i++) {
        ...
        const response = await fetch('/api/excel-import/accessories', {
          method: 'POST',
      ...
      toast({
        ...
        description: `${result.success} accessoires importés avec succès`,
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await fetch('/api/excel-import/accessories/template');
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'template_accessories_import.xlsx';
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <CardTitle>Import d'Accessoires</CardTitle>
          </div>
          ...
          <div className="space-y-6">
            <div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
              ...
              <h3 className="text-lg font-medium mb-2">
                Sélectionnez votre fichier Excel d'accessoires
              </h3>
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>Types d'accessoires supportés:</strong> Masques, Circuits patients, Filtres, Humidificateurs, Batteries, Câbles, etc.
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {mappedData.length} accessoires prêts à être importés
              </p>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      {Object.entries(fieldMapping).map(([fieldKey]) => {
                        const field = accessoryFields.find(f => f.key === fieldKey);
                        return (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {mappedData.slice(0, 10).map((accessory, index) => (
                      <tr key={index}>
                        ...
                        {Object.keys(fieldMapping).map(fieldKey => (
                          <td key={fieldKey} className="px-4 py-3 text-sm text-gray-900">
                            {typeof accessory[fieldKey] === 'boolean' ? (accessory[fieldKey] ? 'Oui' : 'Non') : (accessory[fieldKey] || '-')}
                          </td>
                        ))}
                      </tr>
                    ))}
        ...
        {step === 'importing' && (
          <div className="space-y-6 py-12">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">
                Veuillez patienter pendant l'import des accessoires
              </p>
            </div>
            ...
            <div className="text-center py-8">
              ...
              <div className="space-y-2">
                <p className="text-green-600">
                  <strong>{importResult.success}</strong> accessoires importés avec succès
                </p>
                {importResult.failed > 0 && (
