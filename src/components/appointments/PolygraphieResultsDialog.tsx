...
interface PolygraphieResultsDialogProps {
  appointment: any;
  open: boolean;
...
export function PolygraphieResultsDialog({
  appointment,
  open,
  onOpenChange,
}: PolygraphieResultsDialogProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  // Fetch users for "performed by" selector
  ...
  const [formData, setFormData] = useState<DiagnosticFormData>({
    ...
    followUpDate: '',
    performedById: appointment?.assignedToId || '',
    iah: '',
    idValue: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const diagnosticData = {
      ...
      performedById: formData.performedById || null,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Performed By */}
          <div className="space-y-2">
            <Label htmlFor="performedBy" className="flex items-center gap-2">
              <User className="h-4 w-4" />
              Effectué par
            </Label>
            <Select value={formData.performedById} onValueChange={(val) => updateField('performedById', val)}>
