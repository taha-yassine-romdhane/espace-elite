...
interface MedicalDevicesExcelTableProps {
  devices: MedicalDevice[];
  stockLocations: Array<{ id: string; name: string }>;
  onDeviceUpdate: (device: MedicalDevice) => Promise<void>;
  onDeviceCreate: (device: Partial<MedicalDevice>) => Promise<void>;
...
export function MedicalDevicesExcelTable({
  devices,
  stockLocations,
  onDeviceUpdate,
  onDeviceCreate,
  ...
  const [newDevice, setNewDevice] = useState<Partial<MedicalDevice>>({
    name: "",
    type: "MEDICAL_DEVICE",
    status: "ACTIVE",
  ...
  const handleEdit = (device: MedicalDevice) => {
    ...
    const isCustom = Boolean(device.name && !DEVICES_WITH_CONFIG.includes(device.name));
  ...
  const updateEditedField = (field: keyof MedicalDevice, value: any) => {
    if (editedDevice) {
      setEditedDevice({ ...editedDevice, [field]: value });
    }
  };

  const updateNewField = (field: keyof MedicalDevice, value: any) => {
  ...
  const renderCell = (device: MedicalDevice, field: keyof MedicalDevice, isEditing: boolean) => {
        ...
        case 'technicalSpecs':
          return (
            <Input
              type="number"
              step="0.1"
              value={editedDevice.technicalSpecs || ''}
              onChange={(e) => updateEditedField('technicalSpecs', e.target.value)}
              className="h-8 text-xs"
              placeholder="Heures"
            />
          );
    ...
    switch (field) {
      ...
      case 'technicalSpecs':
        return <span className="text-xs">{value ? `${value}h` : '-'}</span>;
      default:
      ...
      <tr className="bg-blue-50 border-b border-blue-200">
        ...
        <td className="px-2 py-2">
          <Input
            type="number"
            step="0.01"
            value={newDevice.rentalPrice || ''}
            onChange={(e) => updateNewField('rentalPrice', e.target.value ? parseFloat(e.target.value) : null)}
            className="h-8 text-xs w-28"
        ...
        <td className="px-2 py-2">
          <Input
            type="number"
            step="0.1"
            value={newDevice.technicalSpecs || ''}
            onChange={(e) => updateNewField('technicalSpecs', e.target.value)}
            className="h-8 text-xs w-32"
            placeholder="Heures"
          />
        </td>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-lg font-semibold">Appareils Médicaux</h2>
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="bg-gray-100 sticky top-0 z-20">
            <tr className="border-b">
              ...
              <th className="px-2 py-3 text-left text-xs font-semibold min-w-[140px]">Nom</th>
              ...
              <th className="px-2 py-3 text-left text-xs font-semibold min-w-[120px]">Prix Location</th>
            ...
            {paginatedDevices.map((device) => {
              ...
              return (
                <tr key={device.id} className={`border-b hover:bg-gray-50 ${isEditing ? 'bg-yellow-50' : ''}`}>
                  ...
                  <td className="px-2 py-2">{renderCell(device, 'rentalPrice', isEditing)}</td>
                  ...
                  <td className="px-2 py-2">{renderCell(device, 'technicalSpecs', isEditing)}</td>
      ...
      {devices.length === 0 && !isAddingNew && (
        <div className="text-center py-8 text-gray-500">
          Aucun appareil médical. Cliquez sur "Ajouter un appareil" pour commencer.
        </div>
      )}
